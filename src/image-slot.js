...
(() => {
  ...
  // Bumped from 1200 → 2800 so full-bleed hero renderings stay crisp on
  // retina + large displays. Sidecar grows accordingly (~0.8-1.5 MB / image).
  const MAX_DIM = 2800;
  ...
  function load() {
    ...
    const fetchP = window.__IMAGE_SLOTS_STATE ? Promise.resolve(window.__IMAGE_SLOTS_STATE) : fetch(STATE_FILE).then((r) => (r.ok ? r.json() : null));
    loadP = fetchP
      .then((j) => {
  ...
  const stylesheet =
    ...
    // In read-only mode (no omelette runtime), the slot has no interactive
    // features — pointer-events:none lets mobile touches fall through to
    // the page scroll container instead of being captured by shadow-DOM
    // elements.
    ':host(:not([data-editable])){pointer-events:none}' +
    '.frame{position:absolute;inset:0;overflow:hidden;background:rgba(0,0,0,.04);touch-action:pan-y pan-x}' +
    // .frame img (clipped) and .spill (unclipped ghost + handles) share the
    // same left/top/width/height in frame-%, computed by _applyView(), so the
    // inside-mask crop and the outside-mask spill stay pixel-aligned.
    '.frame img{position:absolute;max-width:none;transform:translate(-50%,-50%);' +
    '  -webkit-user-drag:none;user-select:none;touch-action:pan-y pan-x}' +
    ...
    // During reframe mode, disable touch scrolling on the image so
    // pan/resize gestures are captured instead of scrolling the page.
    ...
    '  cursor:pointer;user-select:none;touch-action:pan-y pan-x}' +
